(function(){
  ...
  function primaryMember(items){return items.find(function(m){return m.is_primary})||items[0]||null}
  function debtService(d){
    if(typeof window.debtProductDescription==='function'&&d&&d.product_sale_id)return window.debtProductDescription(d);
    var appointment=(window.appts||[]).find(function(a){return d&&d.appointment_id&&String(a.id)===String(d.appointment_id)});
    return String(appointment&&appointment.operation||d&&d.description||'Eski borç kaydı');
  }
  function memberDebtLines(member){
    var client=byId(member.client_id),name=client&&client.full_name||'Müşteri',open=debtsForClients([String(member.client_id)]).filter(function(d){return d.status==='open'});
    if(!open.length)return '<small class="debt-group-line"><b>'+esc(name)+'</b> · Açık borç yok</small>';
    return open.map(function(d){return '<small class="debt-group-line"><b>'+esc(name)+'</b> · '+esc(debtService(d))+' · '+formatTry(Number(d.amount||0))+'</small>'}).join('');
  }
  ...
  async function load(){
    ...
    var result=await Promise.all([
      ...
      salonDb.from('debt_account_group_members').select('group_id,client_id,relationship_label,is_primary,created_at')
    ]);
  ...
  window.saveDebtAccountGroup=async function(){
    ...
    var rows=[{group_id:created.data.id,client_id:a,relationship_label:document.getElementById('debtGroupRelationA').value.trim()||null,is_primary:true},{group_id:created.data.id,client_id:b,relationship_label:document.getElementById('debtGroupRelationB').value.trim()||null,is_primary:false}],saved=await salonDb.from('debt_account_group_members').insert(rows).select('group_id,client_id,relationship_label,is_primary,created_at');
  ...
  window.renderDebts=function(){
    ...
    var all=window.remoteDebts||[],open=all.filter(function(d){return d.status==='open'}),linkedIds=new Set(members.map(function(m){return String(m.client_id)})),groupCards=groups.map(function(g){var ms=groupMembers(g.id),ids=ms.map(function(m){return String(m.client_id)}),items=debtsForClients(ids),primary=primaryMember(ms),owner=primary&&byId(primary.client_id),lines=ms.map(memberDebtLines).join('');return '<div class="staff debt-customer debt-open debt-family" onclick="openDebtAccountGroupDetail('+js(g.id)+')"><div class="avatar">'+esc(avatarFor(owner&&owner.full_name||g.name))+'</div><div class="item-main"><strong>'+esc(owner&&owner.full_name||g.name)+'</strong><small>'+esc(g.name)+' · İlişkili toplam borç: '+formatTry(openTotal(items))+'</small>'+lines+'</div><span class="link">Detay ›</span></div>'}).join(''),individual={};
  ...
  window.renderDebtDetail=function(){
    var key=String(window.selectedDebtCustomerKey||'');
    if(key.indexOf('group:')!==0){
      var result=baseDetail.apply(this,arguments),holder=document.getElementById('debtDetailContent'),clientId=key.indexOf('id:')===0?key.slice(3):'',member=clientId&&memberFor(clientId);
      if(manager()&&holder&&clientId&&!document.getElementById('debtDetailRelationAction')){
        var button=document.createElement('button');button.type='button';button.id='debtDetailRelationAction';button.className='fab debt-detail-relation';button.textContent=member?'İlişkili borçları görüntüle':'Kişiyi ilişkilendir';button.onclick=function(){if(member)openDebtAccountGroupDetail(member.group_id);else openDebtAccountGroupModal(clientId)};
        var title=holder.querySelector('.page-title');if(title)title.insertAdjacentElement('afterend',button);else holder.prepend(button);
      }
      return result;
    }
