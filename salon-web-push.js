...
async function sync(){try{var sub=await existing();if(currentUser&&Notification.permission==='granted'){if(!sub)sub=await subscribeCurrent();else await save(sub,true)}}catch(error){console.warn('Web Push eşitleme:',error)}render()}
...
var enter=window.enterApp;window.enterApp=function(){var result=enter.apply(this,arguments);install();sync();var p=new URLSearchParams(location.search);if(p.get('date'))setTimeout(function(){route({appointmentDate:p.get('date'),appointmentId:p.get('appointment')})},0);return result};
